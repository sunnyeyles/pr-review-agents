...
| `model-base-url` | no | the provider's own host | Overrides the provider's API host — a gateway, a proxy, or a compatible endpoint. |
...
Prompt caching is requested on every turn and honoured where the provider
